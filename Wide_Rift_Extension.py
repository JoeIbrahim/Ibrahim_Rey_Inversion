...
Model.outputDir="Wide_Rift_Extension_O"
...
# %%
uppermantle = Model.add_material(name="Upper Mantle", shape=GEO.shapes.Layer(top=-36. * u.kilometer, bottom=-76. * u.kilometer))
...
asthenosphere = Model.add_material(name="Asthenosphere", shape=GEO.shapes.Layer(top=-76. * u.kilometer, bottom=Model.bottom))
...
Model.set_heatFlowBCs(bottom=(0.040 * u.watt / u.metre**2, asthenosphere))
...
# %%
import underworld.function as fn
velocity = 1.073 * u.centimeter / u.year
...
coords_FSE_Mantle = GEO.circles_grid(radius=1.5 * u.kilometer, 
                    minCoord=[Model.minCoord[0], -76.*u.kilometer], 
...
uppermantle.phase_changes = GEO.PhaseChange((Model.temperature > GEO.nd(1600.*u.kelvin)), asthenosphere.index)
...
Model.set_temperatureBCs(top=293.15 * u.degK,
                         ...
                         nodeSets = [(air.shape, 293.15 * u.degK), (asthenosphere.shape, 1603.15 * u.degK)])
# %%
solver = Model.solver
...
Model.run_for(8000000 * u.years, checkpoint_interval=100000. * u.year, restartStep=-1, restartDir="Wide_Rift_Extension_O")
